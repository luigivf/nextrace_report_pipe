...
begin
	...
	create table #gen_asm_projec ([id] int identity(1,1),[gen_asm] int, [project] varchar(100))

	if @count > 0 
	begin

		-- 2.2 descoberta por troca de PN
		insert into #gen_asm_projec
		select distinct [gen_asm_id], [Project]
		from _tb_staging_traceability
		where [Project] <> 'not defined'
		and [gen_asm_id] in (
			select --top (@top)
